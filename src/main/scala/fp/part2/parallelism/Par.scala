...
object Par {
  ...
  def map2_timeout[A, B, C](a: Par[A], b: Par[B])(f: (A, B) => C): Par[C] = (es: ExecutorService) => {
    val af = run(es)(a)
    val bf = run(es)(b)
    Map2Future(af, bf, f)
  }
  ...
  case class Map2Future[A, B, C](a: Future[A], b: Future[B], f: (A, B) => C) extends Future[C] {
    ...
    def get(v: Long, unit: java.util.concurrent.TimeUnit): C = compute(TimeUnit.NANOSECONDS.convert(v, unit))
  ...
  def sequence[A](ps: List[Par[A]]): Par[List[A]] =
    ps.foldRight(unit(List.empty[A])) { case ( p, acc) =>
      map2(p, acc)(_:: _)
    }
  ...
  def map3[A, B, C, D](pa: Par[A], pb: Par[B], pc: Par[C])(f3: (A, B, C) => D): Par[D] =
    map2(map2(pa, pb){case (a, b) => (a, b)}, pc){ case ((a, b), c) => f3(a, b, c) }

  def map4[A, B, C, D, E](pa: Par[A], pb: Par[B], pc: Par[C], pd: Par[D])(f4: (A, B, C, D) => E): Par[E] =
    map2(
  ...
  def map5[A, B, C, D, E, F](pa: Par[A], pb: Par[B], pc: Par[C], pd: Par[D], pe: Par[E])(f5: (A, B, C, D, E) => F): Par[F] =
    map2(
      map2(
      map2(pa, pb){case (a, b) => (a, b)},
      map2(pc, pd){ case (c, d) => (c, d) }
      ...
      case ((a, b, c, d), e) => f5(a, b, c, d, e)d
    }

  /**
    * The law of mapping :
    *     * identity: two expressions are identical or equivalent => map(unit(1))(_ + 1) is equivalent to unit(2).
    *     In order to check that let's implemen a function: equals
    */

  def equals[A](e: ExecutorService)(p1: Par[A], p2: Par[B]): Boolean = p1(e).get == p2(e).get
...
object example {
  ...
  def max(ints: IndexedSeq[Int]): Par[Int] = Par.map(Par.sortPar(unit(ints.toList)))(_.last)

  def getNbWords(paragraphs: List[String]): Par[Int] =
    Par.map(unit(paragraphs))(p => p.flatMap(_.split(" ")).size)
}